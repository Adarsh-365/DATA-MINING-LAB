...
int main()
{
  ...
  struct term * head1=NULL, * last1 = NULL, * newnode;
  while(1)
  {
    ...
    else
    {
      ...
      newnode->next = NULL;
      mpz_inits(newnode -> coefficient, newnode -> degree, NULL);
      mpz_set(newnode -> coefficient, coef);
      mpz_set(newnode -> degree, deg);
      last1 -> next = newnode;
      last1 = newnode;
    }
  }
  ...
  printf("\n\nAddition : ");
  display(add(head1, head2, n));

  printf("\n\nSubstraction : ");
  display(sub(head1, head2, n));

  printf("\n\nMultiplication : ");
  display(mul(head1, head2, n));
  ...
  division(quotient, remainder, head1, head2, n);
  printf("\n\nQuotient : ");
  display(quotient->next);
  printf("\n\nRemainder : ");
  display(remainder->next);
  return 0;
}
